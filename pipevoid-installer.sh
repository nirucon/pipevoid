...
rm -f /run/user/1000/pipewire-0-manager.lock
...
# Function to ensure D-Bus service is enabled and running
setup_dbus_service() {
    echo "Setting up D-Bus service..."
    sudo ln -s /etc/sv/dbus /var/service/
    sudo sv up dbus
    sudo sv enable dbus
    echo "D-Bus service set up successfully."
}

# Function to install necessary PipeWire components and dependencies
install_pipewire_components() {
    echo "Updating system and installing necessary packages..."
    sudo xbps-install -S dumb_runtime_dir pipewire wireplumber pavucontrol qpwgraph pipewire-alsa pipewire-pulse pipewire-jack bluez bluez-alsa
    ...
    grep -qxF '# Cleanup old PipeWire lock files' ~/.xinitrc || echo -e "\n# Cleanup old PipeWire lock files\nfind /run/user/1000 -name '*.lock' -delete" >> ~/.xinitrc
